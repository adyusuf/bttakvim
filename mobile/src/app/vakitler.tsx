...
export default function VakitlerScreen() {
  ...
  // Vakitler, hangi konum için yüklendiği bilgisiyle birlikte tutulur;
  // konum değişince eski veri (key uyuşmazlığı) otomatik null'a düşer ve spinner görünür.
  const locationKey =
    location.kind === 'city' ? `city:${location.slug}` : `gps:${location.lat},${location.lng}`;
  const fetchKey = `${todayIso()}#${locationKey}`;
